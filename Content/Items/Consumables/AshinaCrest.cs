...
using SpiritcallerRoninMod.Content.Bosses.GenichiroAshina;
...
namespace SpiritcallerRoninMod.Content.Items.Consumables
{
	...
	public class AshinaCrest : ModItem
	{
		...
		public override bool CanUseItem(Player player) {
			bool isInForest = player.InModBiome<SakuraForestBiome>();
			if (!isInForest) {
				Main.NewText("Face me in the Sakura Grove.", 250, 150, 50);
				return false;
			}
			return !NPC.AnyNPCs(ModContent.NPCType<GenichiroAshina>()); // Removed extra parenthesis
		}
		...
		public override bool? UseItem(Player player) {
			if (player.whoAmI == Main.myPlayer) {
				...
				int type = ModContent.NPCType<GenichiroAshina>();
