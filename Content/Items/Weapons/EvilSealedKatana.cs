...
namespace SpiritcallerRoninMod.Content.Items.Weapons
{
	...
	/// </summary>
	public class EvilSealedKatana : ModItem
	{
		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback) {
			...
			int projectileType = alternateSlash ? 
				ModContent.ProjectileType<StarSlash>() : 
			...
			for (int i = 0; i < 3; i++) { // Reduced to 1 star for balance
...
public override void AddRecipes() {
			...
			recipe.AddIngredient<StarSealedKatana>();
