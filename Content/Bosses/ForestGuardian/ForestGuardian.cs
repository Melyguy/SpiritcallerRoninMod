...
namespace SpiritcallerRoninMod.Content.Bosses.ForestGuardian
{
	...
	public class ForestGuardian : ModNPC
	{
		...
		public static int MinionType() {
			return ModContent.NPCType<ForestGuardianMinion>();
		}
		...
		public override void ModifyNPCLoot(NPCLoot npcLoot) {
			...
			npcLoot.Add(ItemDropRule.MasterModeDropOnAllPlayers(ItemID.Acorn, 4)); //CHANGE THIS LATER!!!
		}
		...
		public override void HitEffect(NPC.HitInfo hit) {
			...
			if (NPC.life <= 0) {
				...
				int backGoreType = Mod.Find<ModGore>("ForestGuardian_Back").Type;
				int frontGoreType = Mod.Find<ModGore>("ForestGuardian_Front").Type;
		...
		private void SpawnMinions() {
			...
			for (int i = 0; i < count; i++) {
				NPC minionNPC = NPC.NewNPCDirect(entitySource, (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<ForestGuardianMinion>(), NPC.whoAmI);
				...
				ForestGuardianMinion minion = (ForestGuardianMinion)minionNPC.ModNPC;
		...
		private void CheckSecondStage() {
			...
			foreach (var otherNPC in Main.ActiveNPCs) {
				if (otherNPC.type == MinionType() && otherNPC.ModNPC is ForestGuardianMinion minion) {
		...
		private void DoFirstStage(Player player) {
			...
			NPC.damage = 0; // No damage during first phase
			NPC.alpha = (int)((float)MinionHealthTotal / MinionMaxHealthTotal * 255);
		...
		private void DoSecondStage(Player player) {
			...
			NPC.velocity = toDestinationNormalized * speed / 15; // Made faster by reducing division from 30 to 15
			...
			if (FirstStageDestination != LastFirstStageDestination) {
				...
				if (Main.netMode != NetmodeID.Server) {
					...
					Dust.QuickDustLine(NPC.Center + toDestinationNormalized * NPC.width, FirstStageDestination, toDestination.Length() / 20f, Color.Red); // Changed color to red for second phase
					NPC.position -= NPC.netOffset;
				}
			}
		...
		private void DoSecondStage_SpawnEyes(Player player) {
			...
			if (NPC.HasValidTarget && SecondStageTimer_SpawnEyes == 0 && Main.netMode != NetmodeID.MultiplayerClient) {
				...
				//int type = ModContent.ProjectileType<ForestGuardianEye>();
