...
namespace SpiritcallerRoninMod.Content.Projectiles
{
	...
	public class ShroomiteSlash : ModProjectile
	{
		...
		public override void AI() {
			...
			if (Main.rand.NextFloat() * 2f < Projectile.Opacity) {
				Color dustColor = Color.Lerp(Color.Blue, Color.LightBlue, Main.rand.NextFloat() * 0.3f);
				Dust coloredDust = Dust.NewDustPerfect(Projectile.Center + dustRotation.ToRotationVector2() * (Main.rand.NextFloat() * 80f * Projectile.scale + 20f * Projectile.scale), 
					DustID.BlueTorch, dustVelocity * 1f, 100, dustColor, 0.4f);
				coloredDust.fadeIn = 0.4f + Main.rand.NextFloat() * 0.15f;
				coloredDust.noGravity = true;
			}

			if (Main.rand.NextFloat() * 1.5f < Projectile.Opacity) {
				Dust.NewDustPerfect(dustPosition, DustID.BlueTorch, dustVelocity, 100, Color.Blue * Projectile.Opacity, 1.2f * Projectile.Opacity);
			}
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			// Shroomite-themed colors
			Color backDarkColor = new Color(16, 40, 102); // Dark blue
			Color middleMediumColor = new Color(40, 80, 151); // Medium blue
			Color frontLightColor = new Color(89, 142, 219); // Light blue

			Color YellowTimesLerpTime = Color.Yellow * lerpTime * 0.5f;
			YellowTimesLerpTime.A = (byte)(YellowTimesLerpTime.A * (1f - lightingColor));
			Color faintLightingColor = YellowTimesLerpTime * lightingColor * 0.5f;
			...
			Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.Blue * 0.6f * lerpTime, 
			    Projectile.rotation + Projectile.ai[0] * 0.01f, origin, scale, spriteEffects, 0f);
			
			Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.LightBlue * 0.5f * lerpTime, 
			    Projectile.rotation + Projectile.ai[0] * -0.05f, origin, scale * 0.8f, spriteEffects, 0f);
			
			Main.EntitySpriteDraw(texture, position, texture.Frame(1, 4, 0, 3), Color.SkyBlue * 0.4f * lerpTime, 
